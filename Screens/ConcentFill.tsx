...
const ConcentFill = () => {
  ...
  //Get API for the consultation form
  useEffect(() => {
    const fetchConsentForm = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/ConcentForm/concent/consultation/${consultationId}`);
  ...
  const handleNext = async () => {
    Alert.alert(
      "Download PDF?",
      "Do you want to download the PDF before proceeding?",
      [
        {
          text: "No",
          onPress: () => {
            navigation.navigate('Startconsultation', {
              formData: { id, consultationId, answers },
            });
          },
          style: "cancel",
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              navigation.navigate('Startconsultation', { formData: { id, consultationId, answers } });
            } catch (err) {
              console.log(err);
  ...
  return (
    <View className="flex-1 bg-white">
      <ScrollView className="flex-1 px-5 mt-10">
        <Text className="text-center text-xl font-bold mb-6 mt-[10%]">{form?.formName}</Text>

        {form?.questions
          .filter(q => q.parentQuestionId === null) // top-level questions only
          .map(parent => (
            <View key={parent.id} className="bg-gray-50 p-4 rounded-2xl shadow mb-4">
              ...
              {parent.inputType === 'radio' && parent.options?.map(opt => (
                <TouchableOpacity
                  key={opt.id}
                  className="flex-row items-center mb-2"
                  onPress={() => handleRadioSelect(parent.id, opt.optionText)}
                >
                  <View className={`w-5 h-5 border-2 rounded mr-2 ${answers[parent.id] === opt.optionText ? 'bg-primary' : 'bg-white'}`} />
                  <Text>{opt.optionText}</Text>
                </TouchableOpacity>
              ))}

              {parent.inputType === 'choice' && parent.options?.map(opt => (
                <TouchableOpacity
                  key={opt.id}
                  className="flex-row items-center mb-2"
                  onPress={() => handleChoiceToggle(parent.id, opt.optionText)}
                >
                  <View className={`w-5 h-5 border-2 rounded mr-2 ${(answers[parent.id] as string[] || []).includes(opt.optionText) ? 'bg-primary' : 'bg-white'}`} />
                  <Text>{opt.optionText}</Text>
                </TouchableOpacity>
              ))}
              ...
              {form.questions
                ...
                .map(child => {
                  ...
                  // ✅ Only show if parentAnswer matches child's optionId
                  const shouldShow =
                  ...
                  return (
                    <View key={child.id} className="mt-4 pl-4 border-l-2 border-gray-300">
                      <Text className="font-semibold mb-2">{child.questionText}</Text>
                      ...
                      {child.inputType === 'radio' && child.options?.map(opt => (
                        <TouchableOpacity
                          key={opt.id}
                          className="flex-row items-center mb-2"
                          onPress={() => handleRadioSelect(child.id, opt.optionText)}
                        >
                          <View className={`w-5 h-5 border-2 rounded mr-2 ${answers[child.id] === opt.optionText ? 'bg-primary' : 'bg-white'}`} />
                          <Text>{opt.optionText}</Text>
                        </TouchableOpacity>
                      ))}

                      {child.inputType === 'choice' && child.options?.map(opt => (
                        <TouchableOpacity
                          key={opt.id}
                          className="flex-row items-center mb-2"
                          onPress={() => handleChoiceToggle(child.id, opt.optionText)}
                        >
                          <View className={`w-5 h-5 border-2 rounded mr-2 ${(answers[child.id] as string[] || []).includes(opt.optionText) ? 'bg-primary' : 'bg-white'}`} />
                          <Text>{opt.optionText}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
